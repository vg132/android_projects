...
public class FeedDroidDB
{
	...
	public ArrayList<Feed> listFeeds()
	{
		...
		try
		{
			...
			cursor = database.rawQuery("SELECT Id, Name, Url, Active, CategoryId FROM Feed", null);
		...
		{
			LogManager.error("Unable to get feeds", ex);
		}
		finally
		{
			if (cursor != null)
	...
	public ArrayList<Feed> listActiveFeeds()
	{
		...
		{
			database = _databaseHelper.getReadableDatabase();
			cursor = database.rawQuery("SELECT Id, Name, Url, Active, CategoryId FROM Feed WHERE Active=1", null);
		...
		{
			LogManager.error("Unable to get active feeds", ex);
		}
		finally
		{
			if (cursor != null)
	...
	public Feed loadFeed(int id)
	{
		...
		{
			database = _databaseHelper.getReadableDatabase();
			cursor = database.rawQuery("SELECT Id, Name, Url, Active, CategoryId FROM Feed WHERE Id=?", new String[] { "" + id });
		...
		{
			LogManager.error("Unabel to get feed with id: " + id, ex);
		}
		finally
		{
			if (cursor != null)
	...
	public void saveFeed(Feed feed)
	{
		...
		try
		{
			...
			if (feed.getId() > 0)
			{
				database.execSQL("UPDATE Feed SET Name=?, Url=?, Active=?, CategoryId=? WHERE Id=?;", new Object[] { feed.getName(), feed.getUrl(), feed.isActive() ? 1 : 0, feed.getCategoryId(), feed.getId() });
			}
			else
			{
				database.execSQL("INSERT INTO Feed(Name, Url, Active, CategoryId) VALUES(?,?,?,?);", new Object[] { feed.getName(), feed.getUrl(), feed.isActive() ? 1 : 0, feed.getCategoryId() });
			}
		}
		catch (Exception ex)
		{
			LogManager.error("Unable to save station", ex);
		}
		finally
		{
			if (database != null)
	...
	public void deleteFeed(int feedId)
	{
		...
		{
			LogManager.error("Unable to delete feed", ex);
		}
		finally
		{
			if (database != null)
	...
	public void cleanupFeedItems(int type)
	{
		...
		{
			LogManager.error("Unable to clean up feed items from type: '" + type + "'", ex);
		}
		finally
		{
			if (database != null)
	...
	public ArrayList<FeedItem> listDeletedFeedItems(Feed feed)
	{
		if (feed != null)
		{
			return listDeletedFeedItems(feed.getId());
		}
		return new ArrayList<FeedItem>();
	}

	public ArrayList<FeedItem> listDeletedFeedItems(int feedId)
	{
		Cursor cursor = null;
		SQLiteDatabase database = null;
		...
		{
			database = _databaseHelper.getReadableDatabase();
			cursor = database.query("FeedItem", new String[] {}, "Deleted=1 AND FeedId=?", new String[] { feedId + "" }, "", "", "date");
			while (cursor.moveToNext())
			{
		...
		{
			LogManager.error("Unable to list feed items", ex);
		}
		finally
		{
			if (cursor != null)
	...
	public ArrayList<FeedItem> listFeedItems(int feedId)
	{
		...
		{
			database = _databaseHelper.getReadableDatabase();
			cursor = database.query("FeedItem", new String[] {}, "Deleted=0 AND FeedId=?", new String[] { feedId + "" }, "", "", "date");
			while (cursor.moveToNext())
			{
				feedItems.add(new FeedItem(cursor));
			}
	...
	public FeedItem loadFeedItem(int feedItemId)
	{
		...
		{
			LogManager.error("Unable to load feed item", ex);
		}
		finally
		{
			if (cursor != null)
	...
	public void saveFeedItem(FeedItem feedItem)
	{
		if (feedItem != null)
		{
			...
			{
				LogManager.error("Unable to save feed item. Message: ", ex);
			}
			finally
			{
				if (database != null)
	...
	public void deleteFeedItem(FeedItem feedItem)
	{
		if (feedItem != null)
		{
			...
			{
				LogManager.error("Unable to delete feed item", ex);
			}
			finally
			{
				if (database != null)
	...
	public int getUnreadFeedItemCount()
	{
		...
		{
			LogManager.error("Unable to count unread feed items", ex);
		}
		finally
		{
			if (cursor != null)
