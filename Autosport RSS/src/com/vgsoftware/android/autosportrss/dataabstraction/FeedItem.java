...
public class FeedItem extends FeedDroidDB
{
	...
	protected FeedItem(Cursor cursor)
	{
		if (cursor != null)
		{
			...
			_categoryId = cursor.getInt(5);
			_read = cursor.getInt(6) == 0 ? false : true;
			_deleted = cursor.getInt(9) == 0 ? false : true;
			try
			{
				_date = DateFormat.parse(cursor.getString(7));
				_added = DateFormat.parse(cursor.getString(8));
			}
			catch (Exception ex)
			{
				_date = new Date();
				_added = new Date();
				Log.error("Unable to parse feed item dates.", ex);
			}
		}
	}
