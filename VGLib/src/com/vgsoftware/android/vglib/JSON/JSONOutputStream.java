...
public class JSONOutputStream extends OutputStream
{
	 ...
	 * 
	 * @param os
	 *          The output stream that receives the JSON output
	 */
	 ...
	 * 
	 * @param object
	 *          The object to that will be converted to JSON stream.
	 */
	public final void writeObject(final Object object)
	{
		...
		for (Field field : fields)
		{
			...
			{
				Log.e("JSONOutputStream", String.format("Failed serializing field [%s] %s", field.getName(), e.toString()));
				continue;
			}
		...
		if (_os != null)
		{
			...
			{
				Log.e("JSONOutputStream", String.format("Failed writing to output stream: %s", e.toString()));
			}
		}
		else
		{
